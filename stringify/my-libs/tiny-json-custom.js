...
// https://github.com/TinyIO/tiny-json
"use strict";
...
const genArray = (propName, cfg) => {
  ...
  if (schema) {
    ...
    exp = genProperty(propName, `JSON.stringify2(${curPath})`);
    beginChunk = false;
  }
  return exp;
};
...
const genStringify = (propName, cfg) => {
  ...
  switch (type) {
    ...
    case "boolean":
      exp = genProperty(
        propName,
        `(t=${fullPath},t==null?'${defaultVal}':!!(t))`
      );
      beginChunk = false;
      break;
    case "integer":
      exp = genProperty(
        propName,
        `(t=parseInt(${fullPath}),isFinite(t)?t:'${defaultVal}')`
      );
      beginChunk = false;
      break;
    case "number":
    ...
    case "string":
      ...
      break;
    case "any":
      exp = genProperty(propName, `JSON.stringify2(${fullPath})`);
      beginChunk = false;
      break;
    default:
...
const createStringifyCustom = (schema) => {
  fullPath = "o";
  beginChunk = true;
